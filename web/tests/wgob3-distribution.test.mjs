...
test('a clone contains the complete WGOB3 game and compiled runtime', async () => {
  ...
  for (const name of ['scummvm.js', 'scummvm.wasm', 'translations.dat', 'COPYING', 'COPYRIGHT']) {
    assert.ok(runtime.files.some(file => file.name === name), `Missing runtime member: ${name}`);
  }
  for (const [directory, files] of [['game/wgob3/', data.files], ['vendor/wgob3/', runtime.files]]) {
    for (const file of files) {
      const bytes = await read(directory + file.name);
      assert.equal(bytes.length, file.size, directory + file.name);
      assert.equal(createHash('sha256').update(bytes).digest('hex'), file.sha256, directory + file.name);
    }
  }
  assert.deepEqual([...((await read('vendor/wgob3/scummvm.wasm')).subarray(0, 8))], [0, 97, 115, 109, 1, 0, 0, 0]);
});
...
test('application startup and game-library dependencies are bundled', async () => {
  ...
  for (const path of ['game/room/bedroom.webp', 'game/room/clips.json', 'wgob3/player.html', 'wgob3/player.js', 'wgob3/player.css', 'wgob3/saves.js']) {
