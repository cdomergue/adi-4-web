...
import { greenhouseGrowth, matchSolution } from '../public/features/science/greenhouse-rules.js';
...
const solutionEntries = Array.from(
  new Map(
    records
...
).map(([key, values]) => ({ casId: Number(key.split(':')[0]), values }));
...
test('MCAS simulation 2 contains eight complete greenhouse solutions', () => {
  ...
  for (const solution of solutions)
    assert.deepEqual(Object.keys(solution).sort(), ['1', '2', '3', '4', '5']);
});
