...
import {crateEntries} from '../public/room-activities.js';
...
const games=JSON.parse(await readFile(new URL('../public/game/games/catalog.json',import.meta.url)));
...
test('the crate keeps the three playable games and separates unfinished ports without duplicate thumbnails',()=>{
  ...
  assert.deepEqual(available.map(game=>game.route).sort(),['#game/mrmatt1','#game/sokoban','#game/wgob3']);
  const upcoming=crateEntries(catalog.games,games,'upcoming');
  assert.ok(upcoming.length>20);assert.ok(upcoming.every(game=>!game.route));
...
test('radio menus preserve real NUL-terminated titles and include music, ambience and silence',()=>{
  assert.equal(catalog.music.length,17);assert.equal(catalog.ambience.length,11);
  ...
  for(const track of [...catalog.music,...catalog.ambience].filter(track=>track.audio)){
    assert.ok(track.duration>20&&track.duration<65);assert.equal(track.sample_rate,22050);
...
test('a clone includes every extracted panel, thumbnail, font and lossless audio file intact',async()=>{
  ...
  for(const item of [...catalog.music,...catalog.ambience,...catalog.games]){
    for(const field of ['audio','image'])if(item[field])assert.ok(files.has(item[field].split('/').pop()),item[field]);
  }
  for(const required of ['menu.ttf','crate-panel.webp','radio-panel.webp','crate-opening.webp','radio-opening.webp'])assert.ok(files.has(required));
  for(const kind of ['crate','radio']){
    ...
    assert.ok(actor.idle.some(sequence=>sequence.clips.join(',')===(kind==='crate'?
      'ADIPZD24,ADIPZD25,ADIPZD26':'ADIPZD56,ADIPZD57,ADIPZD58')));
  }
});
