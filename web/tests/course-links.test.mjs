...
import {parseOriginalLink,findOriginalDefinition} from '../public/course-links.js';
import {showOriginalMedia} from '../public/encyclopedia.js';
...
test('every dictionary link outside the unused original sample page has a definition',()=>{
  ...
  for(const [pageKey,page] of Object.entries(catalog.pages)){
    ...
    for(const [,raw] of page.html.matchAll(/data-media="([^"]*)"/g)){
      ...
      if(type===7){assert.notEqual(findOriginalDefinition(catalog.dictionary,id),null,`${pageKey}: ${raw}`);count++;}
    }
  }
...
test('a course image link opens the requested edition and missing entries do not switch editions',async()=>{
  ...
  globalThis.fetch=async()=>({ok:true,json:async()=>({
    'Adi410Sci65/SINSPEXP':{title:'Cage thoracique',image:'/cage.jpg',caption:'Inspiration et expiration.'},
    'Adi410Sci43/ONLY43':{title:'Autre édition',image:'/other.jpg'}
  })});
  try{
    await showOriginalMedia('SINSPEXP 2','Adi410Sci65',(...args)=>calls.push(args),()=>assert.fail('Not a text page'));
    assert.equal(calls[0][0],'Cage thoracique');
    assert.match(calls[0][1],/src="\/cage.jpg"/);
    await showOriginalMedia('ONLY43 1','Adi410Sci65',(...args)=>calls.push(args),()=>{});
