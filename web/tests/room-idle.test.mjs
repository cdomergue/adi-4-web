...
import { startRoomIdle } from '../public/features/room/idle.js';
...
test('stopping idle playback cannot overwrite an explicitly selected animation', (t) => {
  ...
  fire();
  assert.ok(image.src.endsWith('gesture.webp'));
  image.onload();
  assert.equal(pending.size, 1);
  stop();
  image.src = 'clicked-animation.webp';
...
test('reduced motion and removed actors do not start another gesture', (t) => {
  ...
  const image = { isConnected: true, src: 'base.webp' },
    actor = { idle: [{ file: 'gesture.webp', duration: 1000 }] };
  ...
  callbacks[0]();
  assert.equal(image.src, 'base.webp');
