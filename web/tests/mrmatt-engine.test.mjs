...
import {startLevel, move, won, replayMoves, WIDTH, HEIGHT, TILE} from '../public/mrmatt-engine.js';

const data = JSON.parse(readFileSync(new URL('../public/game/mrmatt1/levels.json',import.meta.url)));
test('all 25 original SOX solutions finish in exactly their recorded number of moves', () => {
  let count = 0;
  for (const pack of data.packs) for (const level of pack.levels) {
    ...
    for (const direction of level.solution.moves) assisted = move(assisted,direction,{noStupidMoves:true});
...
test('save routes round-trip and reject corruption or impossible moves', () => {
  const level = data.packs[0].levels[0], route = level.solution.moves.slice(0,30);
  assert.deepEqual(replayMoves(level,JSON.parse(JSON.stringify(route))),replayMoves(level,route));
  for (const bad of [null,{},'L<script>', 'D'.repeat(16385)]) assert.throws(() => replayMoves(level,bad));
