...
export function startRoomIdle(image, actor, base, {reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches}={}){
  if(reducedMotion||!actor.idle?.length)return ()=>{};
  let previous=-1,timer,stopped=false;
  ...
  function play(){
    ...
    image.onload=()=>{
      ...
      timer=setTimeout(()=>{
        if(!alive())return;
        image.src=base; schedule();
      },clip.duration);
    };
    image.onerror=()=>{image.onload=null;image.onerror=null;image.src=base;};
    image.src='/game/room/activities/'+clip.file;
  }
  schedule();
  return ()=>{stopped=true;clearTimeout(timer);image.onload=null;image.onerror=null;};
}
