...
export function crateEntries(original, games, filter = 'all') {
  const known = new Map(games.map((game) => [game.id, game])),
    seen = new Set(),
    result = [];
  for (const item of original) {
    if (seen.has(item.id)) continue;
    seen.add(item.id);
    result.push({ ...item, route: known.get(item.id)?.route });
  }
  for (const game of games) if (!seen.has(game.id)) result.push(game);
  return result.filter(
    (game) => filter === 'all' || Boolean(game.route) === (filter === 'available'),
  );
}
...
export async function renderRoomActivity(main, kind, info) {
  ...
  try {
    const responses = await Promise.all([
      ...
      ...(radio ? [] : [fetch('/game/games/catalog.json')]),
    ]);
    ...
    const key = radio ? 'radio' : 'crate',
      panel = catalog.artwork[key],
      actor = catalog.artwork[key + 'Actor'];
    const heading = root.querySelector('.scene-heading');
    root.replaceChildren(heading);
    const controls = document.createElement('div');
    controls.className = 'room-controls';
    controls.innerHTML = radio
      ? '<button class="button secondary" id="radio-categories" hidden>← Musiques et ambiances</button>'
    ...
    frame.className = `scene-frame activity-frame ${key}-activity`;
    frame.innerHTML = `<img src="/game/room/image-${radio ? 9 : 26}.webp" width="640" height="480" alt="${radio ? 'Le poste de radio original' : 'La caisse de jeux originale'}">
      ...
      ${[
        ['#welcome', 'Accueil', 'BARINTER'],
        ['#scene/station', 'Sciences', 'BARAPPLI'],
        ['#games', 'Les jeux', 'BARJEUX'],
      ]
        .map(
          ([url, label, icon]) =>
            `<a href="${url}" aria-label="${label}" title="${label}"><img src="/game/room/${icon}.webp" alt=""></a>`,
        )
        .join('')}
      ${[
        ['Les outils', 'BAROUTIL'],
    ...
    pageLabel.className = 'quiet';
    root.append(pageLabel);
    const audio = document.createElement('audio');
    audio.controls = true;
    audio.preload = 'none';
    audio.hidden = true;
    audio.setAttribute('aria-label', 'Lecture de la radio');
    root.append(audio);
    const stop = document.createElement('button');
    stop.className = 'button secondary radio-stop';
    stop.textContent = 'Arrêter la radio';
    stop.hidden = true;
    root.append(stop);
    let mode = 'categories',
      page = 0,
    ...
    const up = frame.querySelector('.activity-up'),
      down = frame.querySelector('.activity-down'),
      scroll = frame.querySelector('.activity-scroll');
    const categories = [
      { id: 'music', title: 'Les musiques', image: assets + 'music.webp' },
      { id: 'ambience', title: 'Les ambiances', image: assets + 'ambience.webp' },
    ];
    const entries = () =>
      radio
        ? mode === 'categories'
          ? categories
          : catalog[mode]
        : crateEntries(catalog.games, games, filter);
    const stopAudio = () => {
      playSerial++;
      audio.pause();
    ...
    audio.onended = () => {
      status.textContent = `${current?.title || 'Morceau'} : lecture terminée.`;
    };
    audio.onerror = () => {
      if (audio.hasAttribute('src'))
        status.textContent =
          'Ce morceau n’a pas pu être chargé. Tu peux réessayer ou en choisir un autre.';
    };
    const select = async (item) => {
      if (help) {
        status.textContent = radio
          ? 'Clique sur une vignette pour écouter, puis utilise les commandes sous la radio. Les flèches font défiler les choix.'
          : item.route
            ? `${item.title} : clique pour jouer.`
            : `${item.title} : ce jeu reste à recréer.`;
        return;
      }
      if (!radio) {
        if (item.route) location.hash = item.route;
          ...
          info(
            item.title,
            '<p>Ce jeu original est identifié. Sa recréation web reste à réaliser.</p>',
          );
        return;
      }
      if (mode === 'categories') {
      ...
      grid
        ...
        .forEach((button) =>
          button.setAttribute('aria-pressed', String(button.dataset.choice === item.id)),
        );
      try {
        await audio.play();
      } catch {
        if (serial === playSerial && root.isConnected)
          status.textContent = 'Clique sur Lecture sous la radio pour démarrer le son.';
      }
    };
    function draw() {
      const items = entries(),
      ...
      grid.classList.toggle('category-items', radio && mode === 'categories');
      ...
      for (const item of items.slice(page * 6, page * 6 + 6)) {
        ...
        button.setAttribute('aria-label', item.title + (!radio && !item.route ? ' — à venir' : ''));
        button.title = item.title;
        if (radio && mode !== 'categories')
      ...
      up.disabled = page === 0;
      down.disabled = page === pages - 1;
      scroll.max = String(pages - 1);
      scroll.value = String(page);
      scroll.disabled = pages === 1;
      ...
      if (!current)
        status.textContent = radio
          ? 'Choisis une vignette pour écouter la radio.'
          : 'Choisis une vignette pour jouer. Les flèches à droite font défiler les jeux.';
    }
    up.onclick = () => {
      page--;
      draw();
    ...
    frame.querySelector('#activity-help').onclick = (e) => {
      ...
      status.textContent = help
        ? 'Aide activée : clique sur une vignette pour lire son rôle. Clique sur ? pour quitter l’aide.'
        : 'Aide désactivée.';
    };
    if (radio)
      controls.querySelector('#radio-categories').onclick = () => {
        mode = 'categories';
        page = 0;
        draw();
      };
    else
      controls.querySelector('#crate-filter').onchange = (e) => {
    ...
    if (root.isConnected)
      root.innerHTML =
        '<p role="status">Ce menu n’a pas pu être chargé. <a href="#room">Revenir dans la chambre</a></p>';
  }
}
