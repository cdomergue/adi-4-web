...
export async function renderMrMatt(main) {
  ...
  try {
    ...
    atlas = new Image();
    atlas.src = '/game/mrmatt1/tiles.webp';
    await atlas.decode();
  } catch {
    if (root.isConnected)
      root.querySelector('[role=status]').textContent =
        'Impossible de charger Mr. Matt. Recharge la page pour réessayer.';
    return;
  }
  ...
  try {
    ...
    if (raw) {
      ...
      if (storage.version === 2 && typeof storage.protection === 'boolean')
        protection = storage.protection;
      const valid = new Set(data.packs.flatMap((p) => p.levels.map((l) => `${p.id}/${l.id}`)));
      if (Array.isArray(storage.completed))
        completed = new Set(storage.completed.filter((id) => valid.has(id)));
      recovery = 'Ta partie a été retrouvée.';
    }
  } catch {
    recovery = 'La sauvegarde n’a pas pu être lue. Une nouvelle partie est prête.';
  }
  ...
  function play(event) {
    ...
    audio.pause();
    audio.src = `/game/mrmatt1/audio/${soundFiles[event]}.wav`;
    audio.play().catch(() => {});
  }
  function persist() {
    try {
    ...
    $('#matt-save-status').textContent = saved
      ? 'Partie et clichés enregistrés automatiquement dans ce navigateur.'
      : 'Stockage indisponible : ta partie reste accessible pendant cette session.';
  }
  function stopWalk() {
  ...
  function fillLevels() {
    ...
    $('#matt-level').innerHTML = pack.levels
      .map((l) => `<option value="${l.id}">${l.id} · ${esc(l.name)}</option>`)
      .join('');
  ...
  function draw(message) {
    ...
    for (let i = 0; i < display.cells.length; i++) {
      ctx.drawImage(
        atlas,
        tileColumn[display.cells[i]] * 16,
        (theme - 1) * 16,
        16,
        16,
        (i % WIDTH) * 16,
        Math.floor(i / WIDTH) * 16,
        16,
        16,
      );
    }
    ...
    if (demonstration)
      status.textContent = won(display)
        ? 'Solution terminée. Reprends ta partie quand tu veux.'
        : `Solution originale · ${demonstration.cursor} / ${level.solution.count} pas`;
    else if (state.dead)
      status.textContent = 'Matt est écrasé ! Annule le dernier pas ou recommence.';
  ...
  function step(direction) {
    ...
    if (route.length >= 16384) {
      draw('Limite de déplacements atteinte. Recommence ce niveau.');
      return false;
    }
    ...
    const event = ['kill', 'win', 'pick', 'crate', 'stone', 'push', 'dig'].find((e) =>
      state.events.includes(e),
    );
    play(event);
    persist();
    draw('À toi de jouer !');
  ...
  function undo() {
    stopWalk();
    if (demonstration || !route.length) return;
    route = route.slice(0, -1);
    state = replayMoves(level, route);
    persist();
    draw('Dernier pas annulé.');
  }
  $('#matt-pack').onchange = (event) => {
    pack = data.packs.find((p) => p.id === event.target.value);
    reset(pack.levels[0]);
  };
  $('#matt-level').onchange = (event) =>
    reset(pack.levels.find((l) => l.id === Number(event.target.value)));
  $('#matt-theme').value = String(theme);
  $('#matt-theme').onchange = (event) => {
    theme = Number(event.target.value);
    persist();
    draw();
  };
  $('#matt-protection').checked = protection;
  $('#matt-protection').onchange = (event) => {
    protection = event.target.checked;
    persist();
  };
  $('#matt-sound').checked = sound;
  $('#matt-sound').onchange = (event) => {
  ...
  $('#matt-restart').onclick = () => {
    reset();
    canvas.focus({ preventScroll: true });
  };
  $('#matt-next').onclick = () => {
    reset(pack.levels[level.id]);
    canvas.focus({ preventScroll: true });
  };
  $('#matt-snapshot').onclick = () => {
    stopWalk();
    snapshot = route;
    play('snapshot');
    persist();
    draw('Cliché enregistré. Tu peux le restaurer à tout moment.');
  };
  ...
  $('#matt-solution').onclick = () => {
    ...
    demoTimer = setInterval(() => {
      if (!root.isConnected || !demonstration) {
        stopDemo();
        return;
      }
      if (document.hidden) return;
      const d = demonstration;
      d.state = move(d.state, level.solution.moves[d.cursor++], { noStupidMoves: false });
      draw();
      if (d.cursor >= level.solution.moves.length) {
        clearInterval(demoTimer);
        demoTimer = null;
      }
    }, 100);
  ...
  $('#matt-fullscreen').onclick = () => {
    ...
    if (document.fullscreenElement === window) document.exitFullscreen?.().catch(() => {});
      ...
      window
        .requestFullscreen?.()
        .catch(() => draw('Le plein écran est indisponible dans ce navigateur.'));
  };
  canvas.onkeydown = (event) => {
    ...
    if (direction) {
      ...
      stopDemo();
      draw('Ta partie est reprise.');
    } else if (/^[1-9]$/.test(event.key) && pack.levels[Number(event.key) - 1]) {
      event.preventDefault();
      reset(pack.levels[Number(event.key) - 1]);
    }
  };
  ...
  canvas.onclick = (event) => {
    ...
    const px = state.player % WIDTH,
      py = Math.floor(state.player / WIDTH);
    if (
      x < 0 ||
      x >= WIDTH ||
      y < 0 ||
    ...
    const destination = y * WIDTH + x,
      direction = x === px ? (y < py ? 'U' : 'D') : x < px ? 'L' : 'R';
    const walk = () => {
      if (!root.isConnected || document.hidden || state.player === destination || !step(direction))
        stopWalk();
    };
    walkTimer = setInterval(walk, 100);
    walk();
  };
  ...
  fillLevels();
  persist();
  draw(recovery || 'Clique sur le plateau puis utilise les flèches.');
}
