...
export async function renderSimulation(main, id) {
  document.title = 'Expérience Sciences · ADI 4';
  main.innerHTML =
    '<section id="simulation"><p role="status">Ouverture de l’expérience…</p></section>';
  const root = main.firstElementChild;
  let data, assets;
  try {
    [data, assets] = await Promise.all([
      get(`/game/station/sim-${id}.json`),
      get('/game/station/assets.json'),
    ]);
  } catch (e) {
    if (root.isConnected)
      root.innerHTML =
        '<p>Cette expérience est introuvable.</p><a href="#simulations">Toutes les expériences</a>';
    return;
  }
  if (!root.isConnected) return;
  ...
  const initial = () => {
    ...
    selected = Object.fromEntries(
      data.objects
        ...
        .map((o) => [o.id, c.initial[o.id] || o.options[0].id]),
    );
  };
  root.innerHTML = `<div class="scene-heading"><a class="back-link" href="#scene/${sector}">← ${sectorNames[sector] || 'La station'}</a><h1>${esc(data.title)}</h1><a class="back-link" href="#simulations">Toutes les expériences →</a></div>
    ...
    <div class="scene-frame simulation-frame" aria-label="${esc(data.title)}"><div class="simulation-layers"></div>${controls
      .filter((o) => o.box && o.box.every((n) => n >= 0))
      .map(
        (o) =>
          `<button class="scene-hotspot" data-object="${o.id}" aria-label="Régler ${esc(o.label)}" style="${position(o.box)}"><span>${esc(o.label)}</span></button>`,
      )
      .join('')}</div>
  ...
  function update() {
    const result = calculateSimulation(data, selected);
    ...
    for (const o of [...data.objects].sort((a, b) => a.plan - b.plan)) {
      ...
      const s = o.options.find((s) => s.id === result.states[o.id]);
      add(assets[s?.visual], o.id);
    }
    ...
    root
      ...
      .forEach((select) => (select.value = selected[select.dataset.input]));
    root.querySelector('#sim-observations').innerHTML = data.objects
      .filter((o) => o.type === 1 && result.resolved.includes(o.id))
      .map((o) => {
        const state = o.options.find((s) => s.id === result.states[o.id]);
        return state?.label ? `<p><strong>${esc(o.label)}</strong> : ${esc(state.label)}</p>` : '';
      })
      .join('');
    root.querySelector('#sim-hints').innerHTML =
      controls
        .map((o) => o.hints[currentCase().id] || o.hints['0'])
        .filter(Boolean)
        .map((t) => `<p>${esc(t)}</p>`)
        .join('') || '<p>Aucun conseil textuel retrouvé pour cette situation.</p>';
    return result;
  }
  root.querySelectorAll('[data-input]').forEach(
    (select) =>
      (select.onchange = () => {
        selected[select.dataset.input] = Number(select.value);
        update();
        root.querySelector('#sim-status').textContent = 'Réglages modifiés.';
        audio.pause();
        root.querySelector('#sim-audio-status').textContent = '';
        const option = controls
          .find((o) => o.id === select.dataset.input)
          .options.find((o) => o.id === Number(select.value));
        if (root.querySelector('#sim-sound').checked && option.audio) {
          audio.src = option.audio;
          audio.play().catch(() => {
            if (root.isConnected)
              root.querySelector('#sim-audio-status').textContent = 'La voix n’a pas pu être lue.';
          });
        }
      }),
  );
  root.querySelector('#sim-sound').onchange = () => audio.pause();
  root.querySelectorAll('[data-object]').forEach(
    (button) =>
      (button.onclick = () => {
        const select = root.querySelector(`[data-input="${button.dataset.object}"]`);
        select.focus();
  ...
  root.querySelector('#sim-case').onchange = () => {
    ...
    initial();
    update();
    root.querySelector('#sim-status').textContent = 'Nouvelle situation : règle les commandes.';
  };
  root.querySelector('#sim-reset').onclick = () => {
    audio.pause();
    initial();
    update();
    root.querySelector('#sim-status').textContent = 'Réglages remis au départ.';
  };
  root.querySelector('form').onsubmit = (e) => {
    ...
    const c = currentCase();
    const known = {};
    for (const key of result.resolved) known[key] = result.states[key];
    const solutions = c.solutions.map((s) =>
      Object.fromEntries(Object.entries(s).filter(([key]) => controls.some((o) => o.id === key))),
    );
    root.querySelector('#sim-status').textContent =
      c.id === '0'
