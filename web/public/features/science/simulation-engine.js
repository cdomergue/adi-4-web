import { calculationEntries, runCalculation } from './generated/original-calculations.js';
...
export function calculateSimulation(data, selected, { sequence, memory = {} } = {}) {
  const states = { ...selected },
    values = {},
    resolved = new Set(),
    objects = new Map(data.objects.map((o) => [Number(o.id), o]));
  ...
  const service = (address) => {
    if (address === 0x35b2) {
      v[0x1d6] = objects.has(v[0x1d3]) ? v[0x1d3] : -1;
      return;
    }
    if (address === 0x3923) {
      const id = String(v[0x1d4]);
      const row = data.lookup.find(
        (r) =>
          r.object === id &&
    ...
    if (address === 0xabab) {
      cache03[v[0x1d4] - 13] = v[0x1d3];
      return;
    }
    if (address === 0xabd3) {
      v[0x1d6] = cache03[v[0x1d3] - 13] ?? 0;
      return;
    }
    ...
    for (let pass = 0; pass < (sequence ? 1 : data.objects.length); pass++) {
      ...
      for (const o of ordered) {
        ...
        for (const [id, value] of Object.entries(pending)) {
          ...
          if (!option) continue;
          if (values[id] !== value || states[id] !== option.id) {
            changed = true;
            values[id] = value;
            states[id] = option.id;
          }
...
export function checkChallenge(solutions, states) {
  return solutions.some(
    (solution) =>
      ...
      Object.entries(solution).every(([id, state]) => states[id] === state),
  );
}
