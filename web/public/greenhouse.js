...
const position = ([x,y,w,h]) => `left:${x/6.4}%;top:${y/4.8}%;width:${w/6.4}%;height:${h/4.8}%`;
export async function renderGreenhouse(main) {
  ...
  try {
    ...
    if(shared.ok){const assets=await shared.json();for(const name of Object.keys(visuals.assets))if(assets[name])visuals.assets[name]=assets[name];}
  } catch {
    if(root.isConnected) root.innerHTML='<p>Les ressources de la serre sont indisponibles.</p><a href="#scene/farm">Retour à l’aire de culture</a>';
    return;
  }
  ...
  const currentCase=()=>data.cases.find(c=>c.id===root.querySelector('#greenhouse-case').value);
  function update() {
    const growth=greenhouseGrowth(values);
    const states={...values,'6':values['3'],'7':growth['7']+1,'8':growth['8']+1,'9':growth['9']+1};
    ...
    for(const id of ['6','1','2','3','4','5','7','8','9']) {
      ...
      if(!asset) continue;
      let img=frame.querySelector(`[data-layer="${id}"]`);const existed=Boolean(img);
      if(!img){img=document.createElement('img');img.alt='';img.className='greenhouse-layer';img.dataset.layer=id;frame.append(img);}
    ...
    root.querySelectorAll('[data-control]').forEach(button=>button.setAttribute('aria-pressed',String(values[button.dataset.control]===Number(button.dataset.state))));
    root.querySelectorAll('[data-setting]').forEach(select=>select.value=values[select.dataset.setting]);
    root.querySelector('#greenhouse-hints').innerHTML=data.controls.map(c=>`<p>${escape(c.hints[currentCase().id]||c.hints['0']||'')}</p>`).join('');
  }
  function set(id,value) {
    values[id]=value;update();result.textContent='Réglages modifiés. Tu peux les vérifier.';
    audio.pause();root.querySelector('#greenhouse-audio-status').textContent='';
    ...
    if(root.querySelector('#greenhouse-sound').checked && option.audio) {
      ...
      audio.play().catch(()=>{if(root.isConnected) root.querySelector('#greenhouse-audio-status').textContent='La voix n’a pas pu être lue.';});
    }
  }
  root.querySelectorAll('[data-control]').forEach(b=>b.onclick=()=>set(b.dataset.control,Number(b.dataset.state)));
  root.querySelectorAll('[data-setting]').forEach(s=>s.onchange=()=>set(s.dataset.setting,Number(s.value)));
  root.querySelector('#greenhouse-case').onchange=()=>{audio.pause();update();result.textContent='Nouveau défi : ajuste tes réglages.';};
  root.querySelector('#greenhouse-sound').onchange=()=>audio.pause();
  root.querySelector('#greenhouse-reset').onclick=()=>{audio.pause();values=Object.fromEntries(data.controls.map(c=>[c.id,1]));update();result.textContent='Réglages remis au départ.';};
  root.querySelector('form').onsubmit=event=>{
    event.preventDefault();
    result.textContent=matchSolution(currentCase().solutions,values)?'Bravo ! Ces réglages correspondent à une solution du défi.':'Pas encore : consulte les conseils d’Adi et essaie une autre combinaison.';
  };
  update();
}
