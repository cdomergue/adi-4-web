...
export const scenes = {
  station: {title:'La station Sciences', back:'room', spots:[
  ...
  biology: {title:'Le centre de biologie',back:'station',spots:[
  ...
  farm: {title:'Aire d’élevage et de culture',back:'biology',spots:[
  ...
  life: {title:'Le Dôme de la vie',back:'biology',spots:[
  ...
  physics: {title:'La physique',back:'biology',spots:[
    {label:'Éclipse de Soleil du 11 août 1999',to:'simulation/13',box:[5,169,226,195]},
    {label:'Conditions d’apparition de la foudre',to:'simulation/3',box:[535,0,105,197]},
  ]},
};

export function renderScene(main,id) {
  const scene=scenes[id];
  const width=scene.width||640, height=scene.height||480;
  document.title=`${scene.title} · ADI 4`;
  const href=to=>to.startsWith('simulation/')||['room','science','encyclopedia','simulations'].includes(to)?`#${to}`:`#scene/${to}`;
