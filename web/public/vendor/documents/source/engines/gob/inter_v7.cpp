...
Inter_v7::Inter_v7(GobEngine *vm) : Inter_Playtoons(vm), _adiAmbientSlot(-1) {
}
...
void Inter_v7::o7_playVmdOrMusic() {
	...
	// ADI uses channel 4 for continuous ambience. The native -20 start
	// command primes frame zero; subsequent frames run while the script polls.
	// A -1 start on that channel closes it, independently of spoken feedback.
	if (_vm->getGameType() == kGameTypeAdi4 && props.lastFrame == -24) {
		if (_adiAmbientSlot >= 0) {
			_vm->_vidPlayer->closeVideo(_adiAmbientSlot);
			_adiAmbientSlot = -1;
		}
		if (props.startFrame == -1 || file.empty())
			return;
		if (props.startFrame == -20 || props.startFrame == -2) {
			props.startFrame = 0;
			props.lastFrame = -1;
			props.noBlock = true;
			props.loop = true;
			_vm->_vidPlayer->evaluateFlags(props);
			_adiAmbientSlot = _vm->_vidPlayer->openVideo(false, file, props);
			if (_adiAmbientSlot >= 0)
				_vm->_vidPlayer->play(_adiAmbientSlot, props);
			return;
		}
	}
