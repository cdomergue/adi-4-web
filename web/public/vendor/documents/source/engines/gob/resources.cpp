...
bool Resources::load(const Common::String &fileName) {
	...
	// ADI document libraries can contain only an external text catalogue.
	// LIBEMM.DAT supplies the media names and dimensions without any sprites.
	const bool hasAdiTexts = _vm->getGameType() == kGameTypeAdi4 &&
		_vm->_dataIO->hasFile(_fileBase + ".dat");
	if (!hasTOTRes && !hasEXTRes && !hasAdiTexts)
		return false;
