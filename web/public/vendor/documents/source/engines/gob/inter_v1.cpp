...
void Inter_v1::o1_strToLong(OpFuncParams &params) {
	...
	uint16 destType;
	destVar = _vm->_game->_script->readVarIndex(nullptr, &destType);
	// ADI packs 16-bit numeric fields next to one another. A 32-bit write
	// erases the following field when parsing its original document tables.
	if (_vm->getGameType() == kGameTypeAdi4)
		writeVar((uint16)destVar, destType, res);
	else
