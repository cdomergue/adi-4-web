...
void Init_v7::initGame() {
	...
	if (ConfMan.hasKey("adi_document")) {
		Common::String module = ConfMan.get("adi_document");
		if (module == "NAVIGA.TOT") {
			_vm->_dataIO->openArchive("atlas.stk", true);
			_vm->_dataIO->openArchive("atlas.itk", true);
		} else {
			_vm->_dataIO->openArchive("simulc.stk", true);
			_vm->_dataIO->openArchive("simulc.itk", true);
		}
	}
	Init::initGame();
}
