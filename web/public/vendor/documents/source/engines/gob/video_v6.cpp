...
void Video_v6::drawPacked(const byte *sprBuf, int16 x, int16 y, Surface &surfDesc) {
	...
	if (*data == 3) {
		int32 size = 0;
		byte *pixels = DataIO::unpack(data + 1, width * height * 3 + 16, size);
		if (size != width * height * 2) error("Invalid RGB555 image size");
		const Graphics::PixelFormat &format = _vm->getPixelFormat();
		// ADI mixes RGB555 resources with paletted VMD scenes. The destination
		// can therefore be indexed even though this resource stores RGB values.
		byte palette[768];
		Graphics::PaletteLookup lookup;
		if (surfDesc.getBPP() == 1) {
			g_system->getPaletteManager()->grabPalette(palette, 0, 256);
			lookup.setPalette(palette, 256);
		}
		for (int row = 0; row < height; ++row) {
			for (int col = 0; col < width; ++col) {
				if (x + col < 0 || y + row < 0 || x + col >= surfDesc.getWidth() || y + row >= surfDesc.getHeight()) continue;
				uint16 c = READ_LE_UINT16(pixels + (row * width + col) * 2);
				byte r = ((c >> 10) & 31) << 3;
				byte g = ((c >> 5) & 31) << 3;
				byte b = (c & 31) << 3;
				uint32 color = surfDesc.getBPP() == 1
					? ((c & 0x7FFF) ? lookup.findBestColor(r, g, b) : 0)
					: format.RGBToColor(r, g, b);
				surfDesc.get(x + col, y + row).set(color);
			}
		}
		delete[] pixels;
		return;
