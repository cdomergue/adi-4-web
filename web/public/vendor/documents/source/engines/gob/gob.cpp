...
extern "C" EMSCRIPTEN_KEEPALIVE void adi_document_sound(int enabled) {
	if (!g_system || !g_system->getMixer()) return;
	for (int i = Audio::Mixer::kPlainSoundType; i <= Audio::Mixer::kSpeechSoundType; ++i)
		g_system->getMixer()->muteSoundType(static_cast<Audio::Mixer::SoundType>(i), !enabled);
}
extern "C" EMSCRIPTEN_KEEPALIVE void adi_document_toolbar() {
	if (!g_system || !g_system->getEventManager()) return;
	Common::Event event;
	event.type = Common::EVENT_MOUSEMOVE;
	event.mouse = Common::Point(320, 479);
	g_system->warpMouse(320, 479);
