...
void DataIO::unpackChunk(Common::SeekableReadStream &src, byte *dest, uint32 size) {
	...
	// ADI RGB555 resources use the extended Coktel LZSS stream: a
	// different initial cursor and an extra length byte for long matches.
	const bool extended = src.readUint32LE() == 0x56781234;
	if (!extended)
		src.seek(-4, SEEK_CUR);
	uint16 tmpIndex = extended ? 273 : 4078;
	memset(tmpBuf, 0, 4114);
	memset(tmpBuf, 0x20, tmpIndex);
