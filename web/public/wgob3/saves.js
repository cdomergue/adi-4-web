...
export function decodeBackup(text) {
  ...
  if (backup.format !== 'adi4-wgob3-saves-v1' || !Array.isArray(backup.files) || backup.files.length > 200) {
    throw new Error('Ce fichier n’est pas une sauvegarde WGOB3.');
  }
  const names = new Set();
  return backup.files.map(file => {
    if (!file || typeof file.name !== 'string' || !/^wgob3\.[a-z0-9_-]{1,30}$/i.test(file.name) || names.has(file.name)) {
...
export function encodeBackup(files) {
  return JSON.stringify({format: 'adi4-wgob3-saves-v1', files: files.map(({name, bytes}) => {
    let binary = '';
